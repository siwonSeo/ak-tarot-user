...
public class TarotService {
    ...
    //상담 정보 수동(로그인시 이력 저장)
    public ResponseTarotCardConsultTotal getTaroCardConsultsBySelf(int cardCount , Boolean isReverseOn
            , Character categoryCode, List<RequestTarotCard.TarotCardSearch> params){
        List<ResponseTarotCardConsult> queryResults = tarotCardRepository.findTaroCardConsults(params);// 쿼리 실행 결과

        log.info("상담 정보 queryResults:{}",queryResults);

        Integer consultId = userService.saveUserConsult(cardCount, isReverseOn,categoryCode,params); //상담이력 저장

        queryResults = this.reOrderConsult(params, queryResults);

        ResponseTarotCardConsultTotal responseTarotCardConsultTotal = new ResponseTarotCardConsultTotal(
                consultId,queryResults
        );

        return responseTarotCardConsultTotal;
    }

    //상담 정보 자동(로그인시 이력 저장)
    public ResponseTarotCardConsultTotal getTaroCardConsultsByRandom(int cardCount , Boolean isReverseOn
